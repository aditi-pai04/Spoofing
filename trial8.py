...
class CNN_LSTM_Model(nn.Module):
    def __init__(self, input_size, cnn_out_channels=64, lstm_hidden_size=128, num_lstm_layers=2, num_classes=1):
        ...
        self.fc = nn.Linear(lstm_hidden_size * 2, num_classes)  # BiLSTM doubles hidden size
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        ...
        x = self.dropout(lstm_out[:, -1, :])  # Last time step output
        x = self.fc(x)
        return self.sigmoid(x)

# Initialize CNN-LSTM Model
input_size = 3
model = CNN_LSTM_Model(input_size)

# Define loss function and optimizer
criterion = nn.BCELoss()

optimizer = optim.AdamW(model.parameters(), lr=0.001, weight_decay=1e-4)
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.5)  # Decay LR every 20 epochs
...
print(f"CNN-LSTM Model Accuracy: {accuracy * 100:.2f}%")
