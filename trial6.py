...
# Define BiLSTM Model for GPS Spoofing Detection
class BiLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2, num_classes=1):
        super(BiLSTMModel, self).__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(hidden_size * 2, num_classes)  # BiLSTM has 2x hidden size output
...
input_size = 3  # Matches PCA output size
...
print(f"BiLSTM Model Accuracy: {accuracy * 100:.2f}%")
