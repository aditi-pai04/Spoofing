...
y_train_tensor = torch.tensor((trace_num % 2).astype(float).values, dtype=torch.float32)

dataset = TensorDataset(X_train_tensor.unsqueeze(1), y_train_tensor)
train_loader = DataLoader(dataset, batch_size=64, shuffle=True)

# Define Hybrid CNN-Transformer Model
class CNN_Transformer_Model(nn.Module):
    def __init__(self, input_size, cnn_out_channels=64, num_heads=2, transformer_layers=2, num_classes=1):
        ...
        self.transformer_layer = nn.TransformerEncoderLayer(d_model=cnn_out_channels, nhead=num_heads)
        self.transformer = nn.TransformerEncoder(self.transformer_layer, num_layers=transformer_layers)
...
input_size = 3
model = CNN_Transformer_Model(input_size)
...
print(f"CNN-Transformer Model Accuracy: {accuracy * 100:.2f}%")
